...
export default class AdminMenu extends DivContainer {

    static _pageList = [];

    addPage(page) {

        AdminMenu._pageList[AdminMenu._pageList.length] = page;
        return this;
    ...
    render() {
        ...
        for (let number in AdminMenu._pageList) {

            let btn = new AdminButton(navigation);
            btn.label = AdminMenu._pageList[number].pageTitle;
            btn.onClick = function () {

                title.text = AdminMenu._pageList[number].pageTitle;

                content.emptyContainer();
                content.addContainer(AdminMenu._pageList[number]);
